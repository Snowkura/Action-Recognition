...
bool Yolov5::Detect(Mat& SrcImg, Net& net, vector<Output>& output) {
	...
	if (!nms_result.empty()) {
		int max_confidence_idx = nms_result[0]; // 假设第一个是最大置信度
		for (int i = 1; i < nms_result.size(); i++) {
			int idx = nms_result[i];
			if (confidences[idx] > confidences[max_confidence_idx]) {
				max_confidence_idx = idx; // 找到更大的置信度，更新索引
			}
		}

		// 只保留最大置信度的结果
		Output result;
		result.id = classIds[max_confidence_idx];
		result.confidence = confidences[max_confidence_idx];
		result.box = boxes[max_confidence_idx];
